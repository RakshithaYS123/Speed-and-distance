        ...
        for i in carLocation1.keys():
            if frameCounter % 1 == 0:
                ...
                if [x1, y1, w1, h1] != [x2, y2, w2, h2]:
                    ...
                    if y1 > 180 and y1 < 285:
                        distance = calculateDistance([x1, y1], [x2, y2])
                        if distance > 0:  # Calculate distance only if objects are moving
                            if x1 < x2:
                                label = "Right"
                            elif x1 > x2:
                                label = "Left"
                                ...
                                label = "Back"
                            cv2.putText(resultImage, f"{label}: {int(distance)} meters", (int(x1 + w1 / 2), int(y1 - 30)),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
